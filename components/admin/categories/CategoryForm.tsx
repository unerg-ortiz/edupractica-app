    ...
    return (
        <div className="bg-slate-900 text-white p-6 rounded-2xl w-full max-w-2xl mx-auto shadow-2xl border border-slate-800">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">{initialData ? t('editTitle') : t('newTitle')}</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('descriptionLabel')}</label>
                    <textarea
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('selectIcon')}</label>
                    <Controller
                        control={control}
                        name="icon"
                        render={({ field }) => (
                ...
                <div className="pt-4 border-t border-slate-800">
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 flex items-center gap-4">
                        ...
                        <div>
                            <h4 className="font-bold text-lg">{nameValue || t('previewName')}</h4>
                            <p className="text-slate-400 text-sm line-clamp-2">{descriptionValue || t('previewDescription')}</p>
                        </div>
                    </div>
                </div>
